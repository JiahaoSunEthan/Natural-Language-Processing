# -*- coding: utf-8 -*-
"""
Created on Wed Jan 26 19:07:42 2022
...
while 1:
    ...
    if not line:
        break
    line = line.split(',')
    model_map[line[0].strip()] = [float(line[1].strip()),float(line[2].strip()),float(line[3].strip()),float(line[4].strip())]
...
for file in file_name_lst:
    f = open(file,'r')
    content = f.read()
    prob_dec,prob_tru,prob_neg,prob_pos = 0,0,0,0
    for word in review_into_wordlst(content):
        if word in model_map:
            prob_dec += model_map[word][0]
            prob_tru += model_map[word][1]
            prob_neg += model_map[word][2]
            prob_pos += model_map[word][3]
    ...
    if prob_dec > prob_tru:
        ...
        label_a = 'truthful'
    result[file] = (label_a,label_b)

fw = open('nboutput.txt', "w")
for key in result.keys():
    fw.write(result[key][0]+' '+result[key][1]+' '+key+'\n')
fw.close()
